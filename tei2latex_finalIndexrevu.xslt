...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- Possibility to change the default lemma separator -->
 ...
 a0paper, a1paper, a2paper, a3paper, a4paper, a5paper, a6paper, b0paper, b1paper, b2paper, b3paper, b4paper, b5paper, b6paper, c0paper, c1paper, c2paper, c3paper, c4paper, c5paper, c6paper, b0j, b1j, b2j, b3j, b4j, b5j, b6j, ansiapaper, ansibpaper, ansicpaper, ansidpaper, ansiepaper, letterpaper, executivepaper, legalpaper -->
  ...
  <xsl:param name="lang1">french</xsl:param>
  ...
  <xsl:param name="rTitleOdd">\textsc{Transcription des coq-à-l'âne}</xsl:param>
  <xsl:param name="rTitleEven">\textsc{Corpus en cours}</xsl:param>
  ...
  <xsl:param name="qmQuote">1</xsl:param>
  ...
  <xsl:param name="fromPage">1</xsl:param>
 ...
 arabic: arabic numerals; roman: lowercase roman numerals; Roman: uppercase roman numerals; alph: lowercase letters; Alph: uppercase letters -->
  ...
  <!-- do you want to print the publication information? Yes if value is different from 0 -->
  <xsl:param name="printPublicationStmt">2</xsl:param>
  ...
  <!-- Do you want to add a prologue to the index locorum? -->
  ...
  <xsl:template match="/">
    ...
    <xsl:if test="$idxNom = '1' and $idxLoc = '1'">
      <xsl:text>
    ...
    <xsl:choose>
      <xsl:when test="$pageNumPlace = 'head'"><xsl:text>
        \fancyhead[</xsl:text><xsl:value-of select="$pageNumPosition"
          /><xsl:text>]{</xsl:text><xsl:value-of select="$pageNumSize"
        /><xsl:text>\thepage}</xsl:text></xsl:when>
      <xsl:otherwise><xsl:text>
        \fancyfoot[</xsl:text><xsl:value-of select="$pageNumPosition"
          /><xsl:text>]{</xsl:text><xsl:value-of select="$pageNumSize"
        /><xsl:text>\thepage}</xsl:text></xsl:otherwise>
    </xsl:choose>
    <xsl:if test="$rTitleEven"><xsl:text>
      \fancyhead[</xsl:text><xsl:value-of select="$rTitleEvenPos"
        /><xsl:text>E]{</xsl:text><xsl:value-of select="$rTitleEvenSize"/><xsl:value-of
        select="$rTitleEven"/><xsl:text>} </xsl:text></xsl:if>
    <xsl:if test="$rTitleOdd != ''"><xsl:text>
      \fancyhead[</xsl:text><xsl:value-of select="$rTitleOddPos"
        /><xsl:text>O]{</xsl:text><xsl:value-of select="$rTitleOddSize"/><xsl:value-of
        select="$rTitleOdd"/><xsl:text>} </xsl:text></xsl:if>
    <!-- Set accordingly  -->
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:text>
      \setmainlanguage{</xsl:text><xsl:value-of select="$lang1"/><xsl:text>}</xsl:text>
      </xsl:otherwise>
    </xsl:choose>
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:text>
      \setotherlanguage{</xsl:text><xsl:value-of select="$lang2"/><xsl:text>}</xsl:text>
      </xsl:otherwise>
    </xsl:choose>
    <xsl:text>
    ...
    <xsl:if test="$lemSep != ''">
      <xsl:text>
      \Xlemmaseparator[]{\,</xsl:text><xsl:value-of select="$lemSep"/><xsl:text>}</xsl:text>
    </xsl:if>
    <!-- to number the lines; options can be: page, pstart or section    -->
    <xsl:text>
    ...
    <xsl:text>
      \sidenotemargin{</xsl:text><xsl:value-of select="$sideNoteLocation"/><xsl:text>} 
        \pagenumbering{</xsl:text><xsl:value-of select="$pageNumberingStyle"/><xsl:text>} </xsl:text>
    <xsl:if test="$fromPage != '1'">
      <xsl:text>
        \setcounter{page}{</xsl:text><xsl:value-of select="$fromPage"/><xsl:text>} </xsl:text>
    </xsl:if>
    ...
    <xsl:if test="$printPublicationStmt != '0'">
    ...
    <xsl:if test="$idxNom = '1'">
      <xsl:if test="$idxNomPrologue != ''"><xsl:text>
        \indexprologue{\small </xsl:text><xsl:value-of select="$idxNomPrologue"
      ...
      <xsl:otherwise>
        <xsl:choose>
          <xsl:when test="not(ancestor::tei:rdg)">
            ...
            <!-- Nota: here we use \textit and not \emph, because this tei:l could be within a tei:quote, which can already be displayed with \emph. This would have the consequence of NOT displaying in italics a tei:lg occurring within a tei:quote -->
            <xsl:text> \\ \indent {</xsl:text>
            <!-- Supprimer la commande \textit pour obtenir un corps de texte en romain -->
          </xsl:when>
  ...
  <!--Activation des ajouts <xsl:template match="tei:add"> </xsl:template>-->
